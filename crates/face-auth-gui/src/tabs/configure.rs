...
impl ConfigureTab {
    pub fn new() -> Self {
        Self { dirty: false, save_status: None }
    }
    ...
    fn draw_fields(&mut self, ui: &mut egui::Ui, config: &mut Config) {
        egui::ScrollArea::vertical().show(ui, |ui| {
            ui.collapsing("Recognition", |ui| {
                egui::Grid::new("recognition_grid").num_columns(2).show(ui, |ui| {
                    ui.label("Threshold");
                    ui.add(egui::DragValue::new(&mut config.recognition.threshold)
                        .speed(0.005).range(0.30..=0.99).fixed_decimals(3));
                    ui.end_row();

                    ui.label("Frames required");
                    ui.add(egui::DragValue::new(&mut config.recognition.frames_required)
                        .range(1..=10));
                    ui.end_row();

                    ui.label("Max enrollment");
                    ui.add(egui::DragValue::new(&mut config.recognition.max_enrollment)
                        .range(1..=100));
                    ui.end_row();
                });
            });
            ...
            ui.collapsing("Daemon", |ui| {
                egui::Grid::new("daemon_grid").num_columns(2).show(ui, |ui| {
                    ui.label("Session timeout (s)");
                    ui.add(egui::DragValue::new(&mut config.daemon.session_timeout_s)
                        .range(3..=60));
                    ui.end_row();
            ...
            ui.collapsing("Liveness", |ui| {
                egui::Grid::new("liveness_grid").num_columns(2).show(ui, |ui| {
                    ui.label("Enabled");
                    ui.checkbox(&mut config.liveness.enabled, "");
                    ui.end_row();

                    ui.label("LBP entropy min");
                    ui.add(egui::DragValue::new(&mut config.liveness.lbp_entropy_min)
                        .speed(0.05).range(0.0..=8.0).fixed_decimals(2));
                    ui.end_row();

                    ui.label("Contrast CV min");
                    ui.add(egui::DragValue::new(&mut config.liveness.local_contrast_cv_min)
                        .speed(0.005).range(0.0..=1.0).fixed_decimals(3));
                    ui.end_row();

                    ui.label("Contrast CV max");
                    ui.add(egui::DragValue::new(&mut config.liveness.local_contrast_cv_max)
                        .speed(0.005).range(0.0..=1.5).fixed_decimals(3));
                    ui.end_row();
                });
            });
            ...
            ui.collapsing("Camera", |ui| {
                egui::Grid::new("camera_grid").num_columns(2).show(ui, |ui| {
                    ...
                    ui.add(egui::Slider::new(&mut config.camera.crop_right_fraction, 0.1..=1.0)
                        .fixed_decimals(2));
                    ui.end_row();
                });
            });
            ...
            ui.collapsing("Notifications", |ui| {
                egui::Grid::new("notify_grid").num_columns(2).show(ui, |ui| {
                    ui.label("Enabled");
                    ui.checkbox(&mut config.notify.enabled, "");
                    ui.end_row();

                    ui.label("Timeout (ms)");
                    ui.add(egui::DragValue::new(&mut config.notify.timeout_ms)
                        .range(0..=30000));
                    ui.end_row();
                });
            });
            ...
            ui.collapsing("Geometry", |ui| {
                egui::Grid::new("geometry_grid").num_columns(2).show(ui, |ui| {
                    ui.label("Distance min");
                    ui.add(egui::DragValue::new(&mut config.geometry.distance_min)
                        .speed(0.005).range(0.01..=0.5).fixed_decimals(3));
                    ui.end_row();

                    ui.label("Distance max");
                    ui.add(egui::DragValue::new(&mut config.geometry.distance_max)
                        .speed(0.005).range(0.1..=2.0).fixed_decimals(3));
                    ui.end_row();

                    ui.label("Yaw max (°)");
                    ui.add(egui::DragValue::new(&mut config.geometry.yaw_max_deg)
                        .speed(1.0).range(10.0..=90.0).fixed_decimals(0));
                    ui.end_row();

                    ui.label("Pitch max (°)");
                    ui.add(egui::DragValue::new(&mut config.geometry.pitch_max_deg)
                        .speed(1.0).range(10.0..=90.0).fixed_decimals(0));
                    ui.end_row();

                    ui.label("Roll max (°)");
                    ui.add(egui::DragValue::new(&mut config.geometry.roll_max_deg)
